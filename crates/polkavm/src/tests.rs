...
fn get_blob(decompress: bool, elf: &'static [u8]) -> ProgramBlob {
...
fn pinky_impl(config: Config, is_64_bit: bool) {
    ...
    let blob = if !is_64_bit {
        get_blob(true, include_bytes!("../../../test-data/bench-pinky_32.elf.zst"))
    } else {
        get_blob(true, include_bytes!("../../../test-data/bench-pinky_64.elf.zst"))
    };
...
const TEST_BLOB_32_ELF: &[u8] = include_bytes!("../../../guest-programs/target/test-blob_32.elf");
const TEST_BLOB_64_ELF: &[u8] = include_bytes!("../../../guest-programs/target/test-blob_64.elf");

impl TestInstance {
    fn new(config: &Config, optimize: bool, is_64_bit: bool) -> Self {
        let _ = env_logger::try_init();
        let blob = if is_64_bit { TEST_BLOB_64_ELF } else { TEST_BLOB_32_ELF };
...
fn test_basic_debug_info(raw_blob: &'static [u8]) {
    let _ = env_logger::try_init();
    let program = get_blob(true, raw_blob);
...
fn test_basic_debug_info_32() {
    test_basic_debug_info(TEST_BLOB_32_ELF);
}

#[ignore]
#[test]
fn test_basic_debug_info_64() {
    test_basic_debug_info(TEST_BLOB_64_ELF);
}
...
fn module_cache(mut config: Config) {
    let _ = env_logger::try_init();
    let blob = get_blob(false, TEST_BLOB_32_ELF);
